/* $Id: model_in_vrml_iv.cxx,v 1.1 2005/05/24 07:37:58 xushun Exp $ */
...
static int skip_iv_field(struct file_data *data)
{
    int c;
                                   ...
                                   int n_faces)
{
  ...
  vnrmls = (vertex_t *)calloc(n_vnrmls,sizeof(*vnrmls));
                                   ...
                                   int max_nidx, int n_faces)
{
  ...
  fnrmls = (vertex_t *)calloc(n_fnrmls,sizeof(*fnrmls));
...
static int read_mfvec3f(vertex_t **a_ref, struct file_data *data)
{
  ...
  n_vtcs = n_vals/3;
  if (n_vtcs > 0) {
    vtcs = (vertex_t *)malloc(sizeof(*vtcs)*n_vtcs);
    if (vtcs != NULL) {
                            ...
                            int *max_vidx)
{
  ...
  n_faces = (n_fidxs+1)/4;
  if (n_faces > 0) {
    faces = (face_t *)malloc(sizeof(*faces)*n_faces);
    if (faces != NULL) {
      for (i=0, j=0; i<n_faces; i++) {
        faces[i].f0 = fidxs[j++];
        faces[i].f1 = fidxs[j++];
                         ...
                         const struct model *inmeshes, int n)
{
  ...
  /* Initialize */
  mesh = (struct model *)calloc(1,sizeof(*mesh));
  ...
  mesh->vertices = (vertex_t *)malloc(sizeof(*(mesh->vertices))*mesh->num_vert);
  mesh->faces = (face_t *)malloc(sizeof(*(mesh->faces))*mesh->num_faces);
  if (w_vtx_nrmls) {
    mesh->normals = (vertex_t *)malloc(sizeof(*(mesh->normals))*mesh->num_vert);
  }
  if (w_face_nrmls) {
    mesh->face_normals = (vertex_t *)malloc(sizeof(*(mesh->face_normals))*mesh->num_faces);
  }
...
int read_vrml_tmesh(struct model **tmeshes_ref, struct file_data *data,
  ...
  do {
    ...
    if (c != EOF) {
      if (n_tmeshes == len) {
        tmeshes = (struct model *)grow_array(tmeshes,sizeof(*tmeshes),&len,SZ_MAX_INCR);
        if (tmeshes == NULL) {
          rcode = MESH_NO_MEM;
          break;
        }
  ...
  if (c != EOF) {
    ...
    do {
      ...
      if (c == '}')
        ...
        if (strcmp(stmp, "Coordinate3") == 0) { /* Coordinate3 */
        ...
        else if(strcmp(stmp, "IndexedTriangleStripSet") == 0){
#ifdef DEBUG
          DEBUG_PRINT("IndexedTriangleStripSet found\n");
#endif
          ...
          do{
            ...
            if (c == '}') { /* end of node */
              ...
              if (strcmp(stmp,"coordIndex") == 0) { /* faces */
                if (n_faces != -1) {
                  rcode = MESH_CORRUPTED;
                } else {
                  n_faces = read_tcoordindex(&faces,data,&max_vidx);
